...
SMSG="/usr/local/bin/smsg"
SMSGWHO="/usr/local/bin/smsgwho"
NETSTAT="netstat"	#just netstat will probably work
MYNAME=`basename $0`
ROOMDIR="/usr/local/var/rc-smsg"	#be sure to include a slash at the end!
DFL_ROOM="dfl"
DFL_HOST="localhost"
DFL_PORT=16354
START_PORT=16355
...
while [ $# -gt 0 ]; do
	...
	-h|-help|--help)
		...
		echo -e "  -a,--alias\t\tspecify alias to use (your username by default)"
		exit 0
		;;
...
normal)
	...
	if [ "$alias" = "" ]; then $SMSG chat -p$port
	else $SMSG chat -p$port -a$alias; fi
