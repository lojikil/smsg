...
.\" - smsg.1.in needed because default portnumber must be generated as well
...
.br
\fBsmsg\fR \fIchat\fR
[\fI--alias \fIALIAS\fR\fR] [\fI--color\fR]  [\fI--port \fIPORT\fR\fR] [\fI--destination \fIHOST\fR\fR]
.br
...
.IP "\fB-a ALIAS\fR, \fB--alias ALIAS\fR"
Used with chat command. Specify a custom \fIalias\fR. Your username will be used
by default.
.IP "\fB-p PORT\fR, \fB--port PORT\fR"
Specifies the target \fIport\fR, if used in conjunction with the server command
it specifies which port to run the server on. The default port is 16354.
